...
class PclDownsampler : public rclcpp::Node
{
  public:
    PclDownsampler() : Node("pcl_downsampler")
    {
      ...
      pclPublisher2= this->create_publisher<sensor_msgs::msg::PointCloud2>("/octomap_read_downsampled", 10);
      pclSubscriber2= this->create_subscription<octomap_msgs::msg::Octomap>("/octomap_read", 1, std::bind(&PclDownsampler::pclDownsamplerCallBack2, this, _1));
    ...
    rclcpp::Subscription<octomap_msgs::msg::Octomap>::SharedPtr pclSubscriber2;
    double leafSize;
    int octo_type;
  ...
  private:
    ...
    void pclDownsamplerCallBack(const octomap_msgs::msg::Octomap::ConstPtr octo_msg) const
    {
      ...
      RCLCPP_INFO(this->get_logger(), "Downsampled Input PCl");

    }

    void pclDownsamplerCallBack2(const octomap_msgs::msg::Octomap::ConstPtr octo_msg) const
    {
      //convert octomap tp pcl
      octomap::OcTree *tree; 
      if (octo_type == 0){
      ...
      // Publish the data
      pclPublisher2->publish(output2);
      RCLCPP_INFO(this->get_logger(), "Downsampled Read PCl");
